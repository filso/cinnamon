// WE NEED TO GET RID OF THIS CRAP
// ReviewMatch is using native lib now
// after we're done with these, remove jQuery-UI
...
var jqyoui = angular.module('ngDragDrop', []).service('ngDragDropService', ['$timeout', '$parse', function($timeout, $parse) {
    ...
    this.move = function($fromEl, $toEl, toPos, duration, dropSettings, callback) {
      ...
      if (toPos === null && $toEl.length > 0) {
        if ($toEl.attr('jqyoui-draggable') !== undefined && $toEl.ngattr('ng-model') !== undefined && $toEl.is(':visible') && dropSettings && dropSettings.multiple) {
          ...
          toPos = $toEl.css({'visibility': 'hidden', 'display': 'block'}).position();
          $toEl.css({'visibility': '','display': wasVisible ? '' : 'none'});
        }
        toPos.left += parseInt($toEl.css('margin-left'));
        toPos.top += parseInt($toEl.css('margin-top'));
      }
      // console.log($('.solutions .part').eq(0).position());
      // console.log($toEl[0]);
    ...
    this.mutateDroppable = function(scope, dropSettings, dragSettings, dropModel, dragItem, jqyoui_pos) {
      ...
      if (angular.isArray(dropModelValue)) {
        ...
        $parse(dropModel + ' = dragItem')(scope);
        if (dragSettings && dragSettings.placeholder === true) {
    ...
    this.mutateDraggable = function(scope, dropSettings, dragSettings, dragModel, dropModel, dropItem, $draggable) {
      ...
      if (dragSettings && dragSettings.placeholder) {
        ...
        if (false && dragSettings.placeholder != 'keep'){
          if (angular.isArray(dragModelValue) && dragSettings.index !== undefined) {
            dragModelValue[dragSettings.index] = dropItem;
          } else {
            $parse(dragModel + ' = dropItem')(scope);
          }
        }
      } else {
        if (angular.isArray(dragModelValue)) {
          ...
          $parse(dragModel + ' = dropItem')(scope);
          if (scope.$parent) {
            $parse(dragModel + ' = dropItem')(scope.$parent);
          }
        }
      }
            ...
            element
              ...
              .draggable({
                start: function(event, ui) {
                  ...
                  jqyoui.startXY = angular.element(this).position();
